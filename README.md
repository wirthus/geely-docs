...
ping android.local
```
Если вы увидите адрес формата IPv6, то выполните команду подключения
```
telnet android.local
```
Если вы увидите адрес формата IPv4, то
...
ifconfig
```
2. Запомните имя интерфейса и адрес IPv6, чтобы отличить адрес вашего ПК от адреса ГУ.
3. Введите команду:
```
ping -I [имя_интерфейса] ff02::1
```
4. Из полученных двух адресов один будет адресом вашего ПК (запомненный в п.2), а другой - адресом ГУ. Скопируйте адрес ГУ.
5. Подключитесь через Telnet, используя полученный адрес:
```
telnet [скопированный_IPv6_адрес]
```
